#!/usr/bin/env python3
"""Headless tests for Unitree G1 example helper behaviors.
...
from examples.example_helpers import g1_model_utils as g1_utils
...
def test_get_retargeting_clips_contains_expected_frames() -> None:
    clips = g1_utils.get_retargeting_clips()
    assert {"reach_cycle", "cross_body", "squat_reach", "side_shift"}.issubset(clips)

    sample = g1_utils.sample_retargeting_clip("reach_cycle", 0.4)
    assert {"right_palm", "left_palm", "right_ankle", "left_ankle", "imu_in_torso"}.issubset(sample)
    assert all(v.shape == (3,) for v in sample.values())


def test_sample_retargeting_clip_interpolates_deterministically() -> None:
    presets = g1_utils.get_retargeting_presets()
    mid = g1_utils.sample_retargeting_clip("reach_cycle", 0.4)
    expected = 0.5 * (presets["neutral"]["right_palm"] + presets["reach_forward"]["right_palm"])
    np.testing.assert_allclose(mid["right_palm"], expected, atol=1e-12)

    end = g1_utils.sample_retargeting_clip("reach_cycle", 999.0)
    np.testing.assert_allclose(end["right_palm"], presets["neutral"]["right_palm"], atol=1e-12)


def test_build_retargeting_target_poses_maps_offsets_from_anchor() -> None:
    anchor = np.eye(4, dtype=float)
    anchor[:3, 3] = np.array([1.0, 2.0, 3.0], dtype=float)
    anchor[:3, :3] = np.array(
        [
            [0.0, -1.0, 0.0],
            [1.0, 0.0, 0.0],
    ...
    poses = g1_utils.build_retargeting_target_poses(
        anchor,
        {"right_palm": np.array([0.2, 0.0, 0.1], dtype=float)},
        scale=0.5,
    )
    np.testing.assert_allclose(poses["right_palm"][:3, 3], np.array([1.0, 2.1, 3.05]), atol=1e-12)
    np.testing.assert_allclose(poses["right_palm"][:3, :3], anchor[:3, :3], atol=1e-12)
...
def test_build_site_mode_target_payloads_counts_and_names() -> None:
    ...
    assert [p["task_name"] for p in payloads_3p] == [
        "g1_site_0",
        "g1_site_1",
        "g1_site_2",
        "g1_site_3",

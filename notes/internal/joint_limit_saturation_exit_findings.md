...
## Iteration 4: Solver Anti-Stall (T1-T6) for Tight Equality-Style Constraints

### Trigger

User-facing G1 floating-base demo (`07_g1_dual_hand_grounded_com_viser.py`) showed:

- frequent `NON_FINITE_INPUT` reports
- repeated `NO_PROGRESS` loops under tight foot epsilon constraints
- motion resumed temporarily when manually widening foot epsilons

### Implemented Solver-Side Techniques

1. **T1 (tight-row headroom):** Inject velocity-box headroom for tight frame/point rows.
2. **T2 (adaptive damping):** Increase regularization when rank-deficient in terminate/direct-solve branch.
3. **T3 (tiny-scale floor):** Keep very small positive scales from collapsing to exact zero (scoped to enhanced tight-row path).
4. **T4 (progressive softening):** Per-row softening factors (tight rows at 4x) widened after repeated zero-scale/no-progress behavior.
5. **T5 (warm-start selector):** Cache and reuse saturated selector matrix for tight-row path when dimensions match.
6. **T6 (pre-solve feasibility):** Conservative rowwise interval feasibility check; pre-soften tight rows before backend solve if infeasible.

### Scope / Guardrails

- To avoid broad solver behavior drift, enhanced logic is activated only when tight-row softening is present.
- Baseline MIN_ERROR and non-tight workloads remain on the original path.
- Scale-floor behavior is explicitly gated to the enhanced path (to avoid oscillation regressions in stress tests).
...
- `pixi run build`
- `pixi run pytest test/test_equality_constraints.py`
- `pixi run pytest test/test_min_error_panda_regression.py`
- `pixi run pytest test/test_panda_narrowed_limits.py::TestMultiSaturationStress::test_stress_strategy_b`
- `pixi run test`
- `timeout 8s pixi run python examples/sungjoon_port_phase1/07_g1_dual_hand_grounded_com_viser.py`

Final results:

- `pixi run test`: **349 passed, 7 skipped, 1 xfailed**
- `test/test_equality_constraints.py`: **8 passed**
- MIN_ERROR panda regression suite: **3 passed**
- Multi-saturation stress guardrail (`strategy_b`) passed after enhanced-path gating
- 07 example smoke launch: Viser server starts cleanly

### Notes on Reverted Intermediate Attempts

- Global `stall_detection_count` increase (2 -> 5) caused regressions outside the target path.
- Unconditional scale-floor induced oscillation in stress tests.
- Always-on enhanced-mode behavior via output pointer plumbing widened impact beyond tight constraints.
- All above were reverted/scoped so the final merged behavior is localized and regression-safe.

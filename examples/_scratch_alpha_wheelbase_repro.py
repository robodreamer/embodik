#!/usr/bin/env python3
"""Local-only optional wheelbase stall reproducer.

This is intentionally a scratch harness for investigation and should not be
committed. It mirrors the validation alpha teleop call pattern closely:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Optional wheelbase stall reproducer (scratch)")
    p.add_argument(
        "--urdf",
        type=Path,
        default=Path(
            "/path/to/local/Projects/validation-repos/validation/validation_sim/assets/sim_robots/official/humanoid/alpha/wheelbase/v2.1/urdf/optional_wheelbase_full.urdf"
        ),
    )
    p.add_argument(
        "--collisions-json",
        type=Path,
        default=Path(
            "/path/to/local/Projects/validation-repos/validation/validation_robot/ros/platforms/optional_wheelbase_description/urdf/collisions.json"
        ),
    )

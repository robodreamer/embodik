#!/usr/bin/env python3
"""Unitree G1 counterpart of Sungjoon 02_ik_site_g1_inspire with Viser.

This version supports two interactive modes:
- Sungjoon 3-point: position-goal style with orientation surrogate points
...
def main() -> None:
    ...
    with server.gui.add_folder("IK Controls"):
        mode = server.gui.add_dropdown(
            "IK Goal Mode",
            options=("Sungjoon 3-point", "EmbodiK 6D"),
            initial_value="Sungjoon 3-point",
        )
    ...
    while True:
        ...
        targets = []
        points = []
        if mode.value == "Sungjoon 3-point":
        ...
        if mode.value == "Sungjoon 3-point" and infeasible_streak >= 8 and ik_r.value:
        ...
        if mode.value == "Sungjoon 3-point" and (not ik_r.value):

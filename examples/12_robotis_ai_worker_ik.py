...
try:
    from example_helpers.robotis_ai_worker_utils import (
        default_worker_allowed_joint_names,
        resolve_ai_worker_frames,
        resolve_ffw_urdf_path,
    )
    from example_helpers.robust_ik_runtime import (
    ...
    if exc.name != "example_helpers" and not str(exc.name).startswith("example_helpers."):
        raise
    from examples.example_helpers.robotis_ai_worker_utils import (
        default_worker_allowed_joint_names,
        resolve_ai_worker_frames,
        resolve_ffw_urdf_path,
    )
...
def main() -> None:
    ...
    with server.gui.add_folder("IK Controls"):
        ...
        solve_mode = server.gui.add_dropdown(
            "Solve Mode", options=("MIN_ERROR", "SCALE"), initial_value="MIN_ERROR"
        )
    ...
    with server.gui.add_folder("Worker Posture"):
        lift_slider = server.gui.add_slider(
            "Lift Joint", -0.5, 0.0, 0.001, _joint_value("lift_joint")
        )
        head_pitch = server.gui.add_slider(
            "Head Pitch", -0.2317, 0.6951, 0.001, _joint_value("head_joint1")
        )
        head_yaw = server.gui.add_slider(
            "Head Yaw", -0.35, 0.35, 0.001, _joint_value("head_joint2")
        )
    ...
    while True:
        ...
        status.value = f"{result.status.name}" + (

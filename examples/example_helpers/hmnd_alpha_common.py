"""Shared helpers for the Optional wheelbase and teststand demos and benchmarks."""
...
def detect_layout(reference_file: Path) -> WorkspaceLayout:
    ...
    validation_repos = projects_root / "validation-repos"
...
class PackagePathResolver:
    ...
    def _build_package_hints(self) -> Dict[str, List[Path]]:
        layout = self._layout
        validation = layout.validation_repos
        models = layout.workspace_root / "validation_robot_models"

        def hints(*candidates: Path) -> List[Path]:
            return [candidate.resolve() for candidate in candidates]

        return {
            "optional_wheelbase_description": hints(
                validation / "validation" / "validation_robot" / "ros" / "platforms" / "optional_wheelbase_description",
                validation / "validation" / "validation_robot" / "install" / "optional_wheelbase_description" / "share" / "optional_wheelbase_description",
                validation / "validation" / "validation_sim" / "assets" / "sim_robots" / "official" / "humanoid" / "alpha" / "wheelbase" / "v1" / "optional_wheelbase_description",
                models / "optional_wheelbase_description",
            ),
            "alpha_teststand_description": hints(
                validation / "validation" / "validation_robot" / "ros" / "platforms" / "alpha_teststand_description",
                validation / "validation" / "validation_robot" / "install" / "alpha_teststand_description" / "share" / "alpha_teststand_description",
                models / "alpha_teststand_description",
            ),
            "alpha_description": hints(
                validation / "validation" / "validation_robot" / "ros" / "platforms" / "alpha_description",
                validation / "validation" / "validation_robot" / "install" / "alpha_description" / "share" / "alpha_description",
                models / "alpha_description",
            ),
            "alpha_biped_description": hints(
                validation / "validation" / "validation_robot" / "ros" / "platforms" / "alpha_biped_description",
                models / "alpha_biped_description",
            ),
            "alpha_ethercat": hints(
                validation / "validation" / "validation_robot" / "install" / "alpha_ethercat",
                validation / "validation" / "validation_robot" / "install" / "alpha_ethercat" / "share" / "alpha_ethercat",
                validation / "validation" / "validation_robot" / "ros" / "platforms" / "alpha_ethercat",
            ),
            "alpha_navigation_sim": hints(
                validation / "validation" / "validation_robot" / "install" / "alpha_navigation_sim",
                validation / "validation" / "validation_robot" / "install" / "alpha_navigation_sim" / "share" / "alpha_navigation_sim",
                validation / "validation" / "validation_sim" / "assets" / "sim_robots" / "official" / "humanoid" / "alpha" / "wheelbase" / "v1",
            ),
            "robotiq_description": hints(
                validation / "validation" / "validation_sim" / "assets" / "sim_robots" / "grippers" / "robotiq" / "robotiq_description",
                validation / "validation" / "validation_robot" / "install" / "robotiq_description",
                validation / "validation" / "validation_robot" / "install" / "robotiq_description" / "share" / "robotiq_description",
            ),
        }
...
def process_xacro_to_temp_urdf(
    ...
    *,
    ...
    prefix = "alpha_teststand_" if "teststand" in str(xacro_path).lower() else "optional_wheelbase_"
...
def create_wheelbase_gripper_xacro(resolver: PackagePathResolver, verbose: bool = False) -> Path:
    ...
    Returns:
        Path to the temporary xacro file (automatically cleaned up on exit)
    """
    wheelbase_package_uri = "package://optional_wheelbase_description/urdf/optional_wheelbase.urdf.xacro"
    ...
    content = f"""<?xml version="1.0"?>
<robot xmlns:xacro="http://www.ros.org/wiki/xacro" name="optional_wheelbase_with_grippers">
...
"""
    temp_file = tempfile.NamedTemporaryFile("w", suffix=".xacro", prefix="optional_wheelbase_gripper_", delete=False)
    temp_file.write(content)
    temp_file.flush()
    temp_file.close()
    temp_path = Path(temp_file.name)
...
def generate_alpha_arm_reachability(
    *,
    ...
    """Generate FK-based reachability map for Optional wheelbase arm.
    ...
    Args:
        xacro_path: Path to Optional wheelbase Xacro file

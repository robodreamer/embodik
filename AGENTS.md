...
- **Build/test/release/gotchas:** See `CLAUDE.md` for commands, architecture, and common gotchas
...
2. **Load context**: check `AGENTS.md`, then `notes/internal/` for prior iteration context, then public docs in `docs/`.
...
5. **Document**: update public docs for user-facing behavior; keep ad-hoc reasoning in `notes/internal/`.
...
| `notes/internal/` | Internal iteration notes/runbooks (not published in MkDocs) |
...
check `notes/internal/` first before re-investigating from scratch. These files are
maintainer-facing context (not public docs) and should be used to inform follow-up work.

Current high-signal notes:

- `notes/internal/joint_limit_saturation_exit_findings.md`
- `notes/internal/personal_installer_gist_notes.md`
- `notes/internal/TASK_TEMPLATE.md`
- `notes/internal/POSTMORTEM_TEMPLATE.md`

Guideline: keep user-facing guidance in `docs/`; keep ad-hoc/internal investigation
artifacts in `notes/internal/`.
...
- Record distilled takeaways in `notes/internal/` so future iterations keep the context.
...
See `CLAUDE.md` for build/test commands and release steps. Always run via `pixi run <task>` so the hermetic environment is used.
...
See `CLAUDE.md` for the full gotchas list.
...
See `CLAUDE.md` for the full-stack change checklist.

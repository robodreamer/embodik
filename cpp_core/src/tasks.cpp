...
static Eigen::Vector3d logMap(const Eigen::Matrix3d& R) {
    ...
    if (std::abs(theta) < 1e-6) {
        ...
        return 0.5 * Eigen::Vector3d(R(2,1) - R(1,2),
                                     R(0,2) - R(2,0),
                                     R(1,0) - R(0,1));
    }

    Eigen::Vector3d w = (1.0 / (2.0 * std::sin(theta))) *
...
FrameTask::FrameTask(const std::string& name,
                     std::shared_ptr<RobotModel> model,
                     const std::string& frame_name,
                     TaskType task_type,
                     int priority,
                     double weight)
    : Task(name, priority, weight),
      model_(model),
    ...
    if (!model_->has_frame(frame_name)) {
        throw std::invalid_argument("Frame '" + frame_name + "' not found in robot model");
    }
}
...
void FrameTask::setTargetPose(const Eigen::Vector3d& position, const Eigen::Matrix3d& rotation) {
    target_position_ = position;
    target_orientation_ = rotation;
    invalidateCache();
}
...
void FrameTask::setTargetPositionVelocity(const Eigen::Vector3d& velocity) {
    if (task_type_ == TaskType::FRAME_ORIENTATION) {
        throw std::runtime_error("Cannot set position velocity for orientation-only task");
    }
...
void FrameTask::setTargetAngularVelocity(const Eigen::Vector3d& omega) {
    if (task_type_ == TaskType::FRAME_POSITION) {
        throw std::runtime_error("Cannot set angular velocity for position-only task");
    }
...
void FrameTask::setTargetVelocity(const Eigen::VectorXd& velocity) {
    ...
    } else if (task_type_ == TaskType::FRAME_ORIENTATION && velocity.size() != 3) {
        throw std::invalid_argument("Orientation task requires 3D angular velocity");
    } else if (task_type_ == TaskType::FRAME_POSE && velocity.size() != 6) {
        throw std::invalid_argument("Pose task requires 6D velocity (linear + angular)");
    }
...
void FrameTask::update(const RobotModel& model) {
    ...
    if (task_type_ == TaskType::FRAME_POSITION || task_type_ == TaskType::FRAME_POSE) {
    ...
    if (task_type_ == TaskType::FRAME_ORIENTATION || task_type_ == TaskType::FRAME_POSE) {
        ...
        switch (task_type_) {
            ...
            case TaskType::FRAME_ORIENTATION: {
                if (!target_orientation_) {
                    ...
                    Eigen::Vector3d error = computeOrientationError(current_orientation_, *target_orientation_);
                    // Apply mask
            ...
            case TaskType::FRAME_POSE: {
                ...
                if (!target_orientation_) {
                    ...
                    Eigen::Vector3d ori_error = computeOrientationError(current_orientation_, *target_orientation_);
...
Eigen::MatrixXd FrameTask::getJacobian() const {
    if (!cache_valid_) {
        ...
        // Following spot_flex_ik_solver.py pattern where excluded joints have zero columns
...
Eigen::Vector3d FrameTask::computeOrientationError(const Eigen::Matrix3d& R_current,
...
COMTask::COMTask(const std::string& name,
                 std::shared_ptr<RobotModel> model,
                 int priority,
                 double weight)
    : Task(name, priority, weight),
...
int COMTask::getDimension() const {
    return 3;
}
...
void PostureTask::setControlledJointTargets(const Eigen::VectorXd& target_values) {
    if (target_values.size() != static_cast<int>(controlled_joint_indices_.size())) {
        throw std::invalid_argument("Target values size must match number of controlled joints");
    }
...
void PostureTask::setControlledJointWeights(const Eigen::VectorXd& weights) {
    if (weights.size() != static_cast<int>(controlled_joint_indices_.size())) {
        throw std::invalid_argument("Weights size must match number of controlled joints");
    }
...
JointTask::JointTask(const std::string& name,
                     std::shared_ptr<RobotModel> model,
    ...
    if (it == joint_names.end()) {
        throw std::invalid_argument("Joint '" + joint_name + "' not found in robot model");
    }

    joint_index_ = std::distance(joint_names.begin(), it) - 1; // -1 because names[0] is "universe"
...
Eigen::MatrixXd JointTask::getJacobian() const {
    return jacobian_;
}
...
MultiJointTask::MultiJointTask(const std::string& name,
    ...
    for (int idx : joint_indices) {
        if (idx < 0 || idx >= model->nv()) {
            throw std::invalid_argument("Joint index " + std::to_string(idx) + " out of bounds");
        }
    }
    ...
    if (target_values.size() == 0) {
        ...
        throw std::invalid_argument("Target values size must match number of joint indices");
    } else {
...
MultiJointTask::MultiJointTask(const std::string& name,
    ...
    for (const auto& joint_name : joint_names) {
        auto it = std::find(model_joint_names.begin(), model_joint_names.end(), joint_name);
        if (it == model_joint_names.end()) {
            throw std::invalid_argument("Joint '" + joint_name + "' not found in robot model");
        }
        int idx = std::distance(model_joint_names.begin(), it) - 1; // -1 for "universe"
    ...
    if (target_values.size() == 0) {
        ...
        throw std::invalid_argument("Target values size must match number of joint names");
    } else {
        target_values_ = target_values;
    }

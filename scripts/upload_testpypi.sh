...
if [ -f "$HOME/.pypirc" ]; then
    ...
    python -m twine upload --repository testpypi --config-file "$HOME/.pypirc" --verbose dist/*.tar.gz 2>&1 || {
        echo ""
    ...
    python -m twine upload --repository testpypi --verbose dist/*.tar.gz
fi
